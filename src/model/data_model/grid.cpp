...
{
    std::array<std::set<Cell*>,9> cell_ptr_rows;
    std::array<std::set<Cell*>,9> cell_ptr_columns;
    std::array<std::set<Cell*>,9> cell_ptr_squares;
    ...
    for(int i = 0; i < 9; i++)
    {
        for(int j = 0; j < 9; j++)
        {
            ...
            cell_ptr_rows[i].insert(cell_ptr);
            cell_ptr_columns[j].insert(cell_ptr);

            square_index = (i/3 * 3) + j/3;
            cell_ptr_squares[square_index].insert(cell_ptr);
        }
    }
